...
fn embed_logo_in_svg(svg: &str) -> String {
    ...
    let img_size = (width.min(height) as f64 * 0.25) as u32; // 25% of QR for the cube
    let cx = width / 2;
    let cy = height / 2;
    let img_x = cx - img_size / 2;
    let img_y = cy - img_size / 2;

    // The PNG already has a white circular background (generated by Pillow)
    // so no need for an extra SVG circle.
    let logo = format!(
        r##"<g>
