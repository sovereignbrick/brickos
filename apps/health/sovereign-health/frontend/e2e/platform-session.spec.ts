 ...
 *
 * Sprint 044 #542: Uses shared auth session from auth.setup.ts.
 * No per-test login -- storageState injected via playwright.config.ts.
 *
 * Run: E2E_BASE_URL=https://demo.brickos.io npx playwright test platform-session
 */

test.describe('BrickOS Platform Admin', () => {
  test('unauthenticated /platform redirects to login', async ({ browser }) => {
    // This test needs a fresh context WITHOUT stored auth
  ...
  test('authenticated /platform shows dashboard', async ({ page }) => {
    await page.goto('/platform')
