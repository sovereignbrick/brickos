...
fn test_standard_protocol() {
    ...
    assert_eq!(resolve_protocol_context("standard", Some("16_8")), "standard");
}

#[test]
fn test_fasting_16_8_protocol() {
    assert_eq!(resolve_protocol_context("fasting", Some("16_8")), "fasting_16_8");
    assert_eq!(resolve_protocol_context("fasting", Some("omad")), "fasting_16_8");
}

#[test]
fn test_fasting_48h_protocol() {
    assert_eq!(resolve_protocol_context("fasting", Some("36h")), "fasting_48h");
    assert_eq!(resolve_protocol_context("fasting", Some("48h")), "fasting_48h");
}

#[test]
fn test_fasting_extended_protocol() {
    assert_eq!(resolve_protocol_context("fasting", Some("72h")), "fasting_extended");
    assert_eq!(resolve_protocol_context("fasting", Some("extended")), "fasting_extended");
}
...
fn test_fasting_default_fallback() {
    ...
    assert_eq!(resolve_protocol_context("fasting", Some("unknown")), "fasting_16_8");
}
...
async fn test_gki_formula() {
    ...
    assert!(gki.is_some(), "GKI should be computed as 5.0, got: {:?}", results);
...
async fn test_homa_ir_formula() {
    ...
    assert!(homa.is_some(), "HOMA-IR should be ~2.224, got: {:?}", results);
}
...
async fn test_missing_inputs_skip_marker() {
    ...
    assert!(results.is_empty(), "Should have no computed markers with only glucose, got: {:?}", results);
}
