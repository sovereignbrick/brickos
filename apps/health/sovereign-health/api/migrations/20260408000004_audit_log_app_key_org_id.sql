...
ALTER TABLE access_log ADD COLUMN IF NOT EXISTS app_key TEXT;
ALTER TABLE access_log ADD COLUMN IF NOT EXISTS org_id UUID;
...
CREATE INDEX IF NOT EXISTS idx_access_log_app_key ON access_log(app_key) WHERE app_key IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_access_log_org_id ON access_log(org_id) WHERE org_id IS NOT NULL;
